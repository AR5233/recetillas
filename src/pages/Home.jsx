    ...
    <div className="min-h-screen bg-[#1A1817] text-base">
      <header className="bg-[#252322] shadow-md sticky top-0 z-20 border-b border-[#3A3633]">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={() => setMenuAbierto(!menuAbierto)} className="text-[#8B7D6B] text-xl w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#2F2C2A]">{menuAbierto ? '✕' : '☰'}</button>
          <h1 className="text-xl font-bold text-[#C8965A] tracking-tight">Recetillas</h1>
         <Link to="/inspirate" className="text-xs bg-[#C8965A]/20 text-[#C8965A] px-3 py-1 rounded-full hover:bg-[#C8965A]/30 transition-colors">✨ Inspírate <span className="text-yellow-500">Beta</span></Link>
        ...
        <div className={`overflow-hidden transition-all duration-300 ${menuAbierto ? 'max-h-96' : 'max-h-0'}`}>
          <div className="max-w-2xl mx-auto px-4 pb-4 space-y-3">
            ...
            <details className="text-[#8B7D6B] text-xs"><summary className="cursor-pointer hover:text-[#C8965A]">Categorías</summary><div className="flex gap-2 flex-wrap mt-2">{CATEGORIAS.filter(c => c !== 'todas').map(c => <button key={c} onClick={() => setFiltro(filtro === c ? 'todas' : c)} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filtro === c ? 'bg-[#C8965A] text-[#1A1817]' : 'bg-[#2F2C2A] text-[#8B7D6B] hover:bg-[#3A3633]'}`}>{c}</button>)}</div></details>
          </div>
        </div>
      </header>
